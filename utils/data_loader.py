...
def load_data(args, data_folder, batch_size, train, num_workers=4, weight_sampler=False, use_fixmatch=False,partial=False,**kwargs):
    ...
    if use_fixmatch:
        transformation = TransformFixMatch(transformation)
...
if __name__ == '__main__':
    data_folder = "/project/transferlearning/data/office_home/Art"
    transformation = transforms.Compose(
                [
                    ...
                    transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])])
    dataset = datasets.ImageFolder(root=data_folder, transform=transformation)
