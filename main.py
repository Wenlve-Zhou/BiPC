...
from utils.tools import str2bool, AverageMeter
...
def get_parser():
    ...
    # network related
    parser.add_argument('--model_name', type=str, default='resnet50',choices=["resnet50", "deit_base_21k", "resnet101",
                                                                              "deit_base", "deit_small", "swint_base",
                                                                              "swint_small", "alexnet"])
    ...
    parser.add_argument('--fixmatch_factor', type=float, default=1.0)
    ...
    # loss related
    parser.add_argument('--clf_factor', type=float, default=1.0)
    parser.add_argument('--cpa_factor', type=float, default=1.0)
    parser.add_argument('--cgi_factor', type=float, default=0.25)
...
def test(model, target_test_loader, args):
    ...
    if args.datasets == "visda":
        acc = sklearn.metrics.balanced_accuracy_score(all_label.cpu().numpy(),
                                                          torch.squeeze(all_pred).float().cpu().numpy()) *100
        cm = sklearn.metrics.confusion_matrix(all_label.cpu().numpy(),
